...
def create_wishlist(name: str,current_user: User = Depends(get_current_user)):
    wishlist = Wishlist(name=name,user_id=current_user.id)
...
def get_my_wishlists(current_user: User = Depends(get_current_user)):
    wishlists = session.query(Wishlist).filter(Wishlist.user_id == current_user.id).all()
...
def delete_wishlist(wishlist_id: str,current_user: User = Depends(get_current_user)):
...
def add_to_wishlist(wishlist_id: str,perfume_id: str,current_user: User = Depends(get_current_user)):
    wishlist = session.query(Wishlist).filter(Wishlist.id == wishlist_id,Wishlist.user_id == current_user.id).first()
    ...
    if already_in_wishlist:
        raise HTTPException(status_code = 409,detail = "Perfume already in wishlist")
...
def remove_from_wishlist(wishlist_id: str,perfume_id: str,current_user: User = Depends(get_current_user)):
    wishlist = session.query(Wishlist).filter(Wishlist.id == wishlist_id,Wishlist.user_id == current_user.id).first()
    ...
    if existing_item is None:
        raise HTTPException(status_code = 404,detail = "Perfume not in wishlist")
...
def get_wishlist(wishlist_id: str,current_user: User = Depends(get_current_user)):
    wishlist = session.query(Wishlist).filter(Wishlist.id == wishlist_id,Wishlist.user_id == current_user.id).first()
